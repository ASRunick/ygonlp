...
ただし、現在実装済みの分析は、まずLength Metricsを中心としています。
...
- 時系列分析は記述的な関連を示すものであり、因果関係を証明するものではない
...
これは、遊戯王カードには短い効果のカードから非常に長い効果を持つカードまで幅広く存在するためです。
...
release count分析を組み合わせることで、この2つを分離して考えられます。
...
1. カード効果テキストをベクトル化する
2. 各カード間のcosine similarityを計算する
3. 類似度の高いカードをランキングする
...
固定seedを利用することで、同じ入力から同じ分析結果を再生成できるようにしています。
